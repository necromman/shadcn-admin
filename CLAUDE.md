...
### LMS 메인 페이지 필수 요소
**LMS(Learning Management System) 사이트 구현 시 반드시 포함해야 할 요소들:**

#### 1. 헤더 구성
- **상단 유틸리티바**: 로그인/로그아웃, 마이페이지, 언어 선택, 기관 선택
- **메인 네비게이션**: 이러닝, 가상훈련, 추천테마, 커뮤니티 등 주요 카테고리
- **검색 기능**: 통합 검색 또는 과정 검색

#### 2. 메인 섹션 구조
- **히어로 배너**: 슬라이드형 주요 공지/이벤트 (자동 재생, 페이지네이션)
- **인기 강좌**: 실시간 인기 과정 카드형 목록
- **추천 과정**: 카테고리별 추천 과정
- **신규 과정**: 최근 등록된 과정
- **공지사항/뉴스**: 학습 관련 공지 및 업데이트
...
- 수강생 수
- 난이도 표시
- 과정 유형 태그 (온라인/오프라인/블렌디드)
- 진행 상태 (모집중/진행중/종료)
- 버튼: 미리보기, 관심과정 담기, 수강신청
```

#### 4. 색상 및 디자인 톤
- **메인 컬러**: 신뢰감 있는 블루 계열 (blue-600, blue-700)
- **보조 컬러**: 그레이 톤으로 계층 구조 표현
- **강조 컬러**: 수강신청/중요 알림용 (orange, green)
- **배경**: 깔끔한 흰색/연한 그레이 (gray-50)

#### 5. 반응형 대응
- **데스크톱**: 4-5개 컬럼 그리드
- **태블릿**: 2-3개 컬럼
- **모바일**: 1-2개 컬럼, 스와이프 가능한 캐러셀

### 작업 관리 문서 운영
**모든 LMS 구현 작업은 다음 문서를 유지관리:**
- `docs/lms-implementation-plan.md`: 구현 계획서
- `docs/lms-work-progress.md`: 작업 진행 상황 추적
- 매 작업 단계마다 진행상황 업데이트 필수

### 이미지 및 아이콘 처리
- **플레이스홀더 이미지는 반드시 https://placehold.co 사용**:
  - 올바른 형식: `https://placehold.co/400x225/3B82F6/FFFFFF/png?text=React`
  - 형식 설명: `https://placehold.co/너비x높이/배경색/텍스트색/형식?text=텍스트`
  - 색상은 HEX 코드 사용 (# 제외)
  - ❌ 절대 사용 금지: `https://via.placeholder.com` (작동하지 않음)
- **로고는 텍스트로만**: "STEP LMS" 등 텍스트 표기
- **아이콘은 lucide-react 사용**: 일관된 아이콘 시스템

### 🚫 배경색 및 그라데이션 엄격 금지 사항
**절대로 다음과 같은 촌스러운 그라데이션 사용 금지:**
- ❌ **절대 금지**: `bg-gradient-to-br from-purple-600 to-blue-600`
- ❌ **절대 금지**: `bg-gradient-to-r from-blue-600 to-purple-700`
- ❌ **절대 금지**: 높은 채도의 그라데이션 배경
- ❌ **절대 금지**: 여러 색상이 섞인 화려한 그라데이션

**대신 사용해야 할 배경 스타일:**
- ✅ **단색 배경**: `bg-blue-600`, `bg-gray-100`
- ✅ **아주 은은한 그라데이션**: `bg-gradient-to-b from-gray-50 to-white`
- ✅ **미묘한 색상 변화**: `bg-gradient-to-br from-blue-50/50 to-indigo-50/50`
- ✅ **깔끔한 단색 + 패턴**: 단색 배경에 subtle한 패턴이나 텍스처
