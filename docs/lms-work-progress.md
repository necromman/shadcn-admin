...
## 2025-01-18: 이러닝 서브페이지 구현

### ✅ 완료된 작업
- [x] **이러닝 서브페이지 구현** (완료)
  - 이러닝 서브페이지 구현 계획 작성 (`docs/lms-elearning-implementation-plan.md`)
  - 헤더 네비게이션에서 이러닝 자식 노드 제거
  - 라우트 파일 생성 (`/elearning`)
  - 공통 서브페이지 레이아웃 컴포넌트 생성 (`SubPageLayout.tsx`)
  - 사이드바 카테고리 컴포넌트 구현 (`CategorySidebar.tsx`)
  - 과정 검색 영역 컴포넌트 구현 (`CourseSearchBar.tsx`)
  - 강의 목록 테이블 형식 구현 (`CourseTable.tsx`)
  - 페이지네이션 컴포넌트 추가 (`Pagination.tsx`)
  - 전체 서브페이지 통합 (`ElearningPage.tsx`)

### 구현된 기능
- **레이아웃**: 왼쪽 사이드바 + 오른쪽 메인 콘텐츠
- **카테고리 필터링**: IT/SW, 디자인, 비즈니스, 외국어 등
- **검색 및 필터**: 과정명/강사명/기관명 검색, 난이도/상태/가격 필터
- **정렬**: 인기순, 최신순, 평점순, 가격순
- **테이블 형식 목록**: 체크박스 선택, 썸네일, 상세정보, 액션 버튼
- **페이지네이션**: 페이지 네비게이션, 페이지당 항목 수 설정
- **반응형**: 모바일에서 사이드바 드로어 방식